...
public interface CenterRepository extends MongoRepository<Center, ObjectId> {
    ...
    // 센터명으로 검색
    @org.springframework.data.mongodb.repository.Query("{ 'cntrNm': { $regex: ?0, $options: 'i' } }")
    List<Center> findByCntrNmRegex(String name);
    
    // 주소로 검색
    @org.springframework.data.mongodb.repository.Query("{ 'cntrAddr': { $regex: ?0, $options: 'i' } }")
    List<Center> findByCntrAddrRegex(String address);

    // 상세주소로 검색
    @org.springframework.data.mongodb.repository.Query("{ 'cntrDaddr': { $regex: ?0, $options: 'i' } }")
